...
An Operating Model is simultaneously executable guidance, a business asset, and runtime input. Its complete lifecycle (storage, collaboration, approval, publication and activation) is application-owned design work. Framework's optional `OperatingModelChangePort` intentionally narrows the seam to applying an already reviewed candidate. It receives candidate digest, bound module / WorkType / path, base digest or Git HEAD, reviewer and requested validation; only an adapter result carrying an actual applied reference and validation evidence permits `reflection.adopted` in canonical trace. With no adapter, adoption is explicitly unavailable while review, download and rejection remain available.
