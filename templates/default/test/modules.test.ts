...
import { digest, ModuleRegistry, stable } from "@airic/framework";
...
describe("application modules", () => {
  it("loads every declared module and WorkType package", async () => {
    ...
    expect(refs).toEqual(expect.arrayContaining([{ moduleId: "cases", workTypeId: "case-assistance" }, { moduleId: "development", workTypeId: "module-smith" }, { moduleId: "development", workTypeId: "operating-model-smith" }, { moduleId: "development", workTypeId: "reflection" }]));
    const { loadWorkDefinition } = await import("@airic/framework");
    for (const ref of refs) { const packagePath = registry.manifest(ref.moduleId).workTypes.find((item) => item.id === ref.workTypeId)!.path; const exported = await source.exportFiles({ ...ref, packagePath }); const files = Object.entries(exported.files).map(([path, content]) => ({ path, content, digest: digest(content) })); const contentDigest = digest(stable(files.map(({ path, content }) => ({ path, content })))); expect((await loadWorkDefinition({ target: ref, ref: { revisionId: `working:${contentDigest.slice(0, 12)}`, contentDigest }, files })).manifest.id).toBe(ref.workTypeId); }
  });
