...
const airic = createAiricHttpHandler({
  runtime, basePath: "/api/airic", authenticate: resolveActor,
...
async function shutdown() {
  await airic.close(); await new Promise<void>((resolveClose) => server.close(() => resolveClose())); await runtime.close();
