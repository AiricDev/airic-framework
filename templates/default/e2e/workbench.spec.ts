...
test("creates and completes a governed transaction in simulated mode", async ({ page }) => {
  ...
  await expect(page.locator(".conversation").getByText(/domain did not accept completion yet/i)).toBeVisible();
  await page.getByLabel("Message").fill("name is Ada Lovelace, ada@example.com");
  await page.getByRole("button", { name: "Send" }).click();
  await expect(page.locator(".conversation").getByText("The domain accepted the transaction and the case is ready.")).toBeVisible();
  await expect(page.locator(".status", { hasText: "completed" })).toBeVisible();
  await expect(page.locator(".case-summary").getByText("Ada Lovelace")).toBeVisible();
