...
The owning Module's outer DomainProvider adapter converts Airic's trusted call context to application input. Never trust actor, scope, command ID, Work ID or authorization copied from tool arguments. A command capability must atomically persist its effect and its stable command receipt, and `inspectCommand` must query that receipt. Map deterministic rejection to a structured rejected receipt; treat transport ambiguity as `unknown`.

Declare exports in `module.yml` and pin `release`, `buildId`, and a reviewed `sourceBundle` digest. Cross-module consumers use only `public/` contracts and consumer-owned ports; they never deep-import or access another module's repository. Let the Agent read relevant domain/use-case/types/tests, excluding secrets and infrastructure. Contract tests must prove schemas, release binding, durable receipt lookup and stale business revision rejection.
