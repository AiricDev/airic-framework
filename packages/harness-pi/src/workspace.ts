...
export async function createWorkspaceTools(input: { policy: WorkspacePolicy; definitionId: string; stateDirectory: string }): Promise<ToolDefinition[]> {
  ...
  if (!workspaceState) {
    ...
    workspaceState = { baseline, lastKnown: { ...baseline } };
  ...
  const readTool = defineTool({
    ...
    execute: async (_id, value) => result(await readFile(await safePath(root, value.path, "read"), "utf8")),
  });
  const listTool = defineTool({
    ...
    execute: async (_id, value) => result((await listFiles(root, value.path ?? ".", value.limit ?? 200)).join("\n")),
  });
  const searchTool = defineTool({
    ...
    execute: async (_id, value) => result((await searchFiles(root, value.query, value.path ?? ".", value.limit ?? 100)).join("\n")),
  });
  ...
  const diffTool = defineTool({
    ...
    parameters: Type.Object({ path: Type.Optional(Type.String()) }), execute: async (_id, value) => result(await gitDiff(root, value.path)),
  });
  const changesTool = defineTool({
    ...
    parameters: Type.Object({}), execute: async () => {
      const changedFiles = await changes(root, grant, state);
  ...
  const checks = (grant.checks ?? []).map((check) => defineTool({
    ...
    execute: async () => {
      ...
      return result(`${stdout}${stderr}`.trim() || `${check.title} passed`, { check: { id: check.id, title: check.title, status: "passed" } });
    },
  }));
...
function isSecret(path: string): boolean {
  ...
  return segments.some((segment) => ignoredSegments.has(segment)) || segments.some((segment) => secretNames.has(segment) || segment.startsWith(".env."));
}
...
async function listFiles(root: string, requested: string, limit: number): Promise<string[]> {
  const start = await safePath(root, requested, "read"); const found: string[] = [];
  async function visit(path: string) {
    if (found.length >= limit) return;
    const info = await lstat(path);
    if (info.isSymbolicLink()) return;
    if (!info.isDirectory()) { found.push(relative(root, path).split(sep).join("/")); return; }
    for (const name of (await readdir(path)).sort()) { const child = resolve(path, name); const rel = relative(root, child).split(sep).join("/"); if (!isSecret(rel)) await visit(child); }
  }
  await visit(start); return found;
}

async function searchFiles(root: string, query: string, requested: string, limit: number): Promise<string[]> {
  const output: string[] = [];
  for (const file of await listFiles(root, requested, 2000)) {
...
async function snapshot(root: string, grant: WorkspaceGrant): Promise<Record<string, string | null>> {
  ...
  for (const writeRoot of grant.write) {
    try { for (const file of await listFiles(root, writeRoot, 5000)) if (!matchesAny(file, grant.denyWrite ?? [])) value[file] = await fileDigest(resolve(root, file)); } catch (error) { if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error; }
  }
  return value;
}

async function changes(root: string, grant: WorkspaceGrant, state: WorkspaceState) {
  const current = await snapshot(root, grant); const paths = new Set([...Object.keys(state.baseline), ...Object.keys(current)]);
  return [...paths].sort().flatMap((path) => state.baseline[path] === current[path] ? [] : [{ path, status: state.baseline[path] == null ? "added" : current[path] == null ? "deleted" : "modified", beforeDigest: state.baseline[path] ?? null, afterDigest: current[path] ?? null }]);
}
...
async function gitDiff(root: string, requested?: string): Promise<string> { const run = promisify(execFile); if (requested) await safePath(root, requested, "read"); const args = ["diff", "--no-ext-diff", "--", ...(requested ? [requested] : ["."])]; const { stdout } = await run("git", args, { cwd: root, maxBuffer: 4 * 1024 * 1024 }); return stdout || "No tracked-file diff."; }
...
async function readState(path: string): Promise<WorkspaceState | undefined> { try { return JSON.parse(await readFile(path, "utf8")) as WorkspaceState; } catch (error) { if ((error as NodeJS.ErrnoException).code === "ENOENT") return undefined; throw error; } }
