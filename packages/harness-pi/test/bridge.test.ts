...
describe("Pi bridge delivery probe", () => {
  it("exposes the rendered envelope, actual tools, hooks and pre-send payload digest", () => {
    const envelope = {
      ...
      instructions: [{ id: "process", title: "Process", content: "Follow reviewed method", digest: "d", authority: "operating-model" }], observations: [], capabilityCatalog: [], availableCapabilities: [], discoverableContent: [], provenance: [], turnContextRefs: [{ namespace: "certreporter", resourceType: "report", resourceId: "report-1", selection: { scope: "clause", clauseId: "clause-7" } }], digest: "envelope-digest",
    ...
    expect(probe.systemPrompt).toContain("Turn context references");
    expect(probe.systemPrompt).toContain("clause-7");
