...
export class AiricRuntime {
  ...
  #toolsFor(work: Work, definition: WorkDefinition, domains: readonly DomainProvider[], actor: TrustedCallContext["actor"], evidenceDigests?: readonly string[]): HarnessTool[] {
    ...
    return [
      ...
      ...(definition.manifest.completion.mode === "agent" ? [
        { name: "airic_complete_work", description: "Complete the Work with a structured result after required domain effects are confirmed.", inputSchema: { type: "object", properties: { result: {} }, required: ["result"] }, invoke: async (value: unknown) => this.#completeWork(work.id, (value as { result: unknown }).result) },
      ] satisfies HarnessTool[] : []),
    ];
  }
